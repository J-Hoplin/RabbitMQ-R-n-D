...
`Publisher`가 `Queue`에 전송한 메세지를 `Consumer`가 소비하기 위해서는 `channel`객체의 `consume()`메소드를 사용하면 된다. 첫번째 매개변수로는, `Queue`의 이름이, 두번째 매개변수는 결과가 처리될 콜백함수를, 세번째 매개변수로는 옵션들을 넘겨준다. 세번째 매개변수인 옵션들은 [Document](https://amqp-node.github.io/amqplib/channel_api.html#channel_consume)를 참고한다.
